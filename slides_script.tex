...
\item Contributions: OpenFreqBench, metric-complete comparison, evidence that compliance-style tests miss IBR failure modes, and RA-EKF as one tested estimator.
...
\item The deck uses 18 estimators, 33 scenarios, and Monte Carlo plus ATLAS sweeps.
...
\item Evidence level matters. A pilot sweep can diagnose a failure mode; a paper-grade sweep supports a stronger claim.
...
\item Across 113 severity levels, RMSE, trip-risk, and RoCoF metrics often select different winners.
\item For protection-adjacent use, low average error is not enough; deadband exposure and RoCoF tracking matter.
