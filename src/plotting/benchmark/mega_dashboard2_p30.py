"""
Subplot (g): MAE vs. CPU latency with Pareto highlighting.
"""
import numpy as np
...
_PARETO_EDGE = "#1B5E20"
_RT_COLOR = "#C62828"
_IEEE_FE_COLOR = "#1565C0"
_IEEE_FE_GUIDE_MHZ = 5.0
...
def md2_subplot_30(ax, data_bundle):
    ...
    mae_arr = grp["mae_mean"].to_numpy(dtype=float) * 1000.0
    cpu_serr = grp["cpu_std"].to_numpy(dtype=float)
    mae_serr = grp["mae_std"].to_numpy(dtype=float) * 1000.0

    pareto = _pareto_mask(cpu_arr, mae_arr)
    ...
    ax.axhline(_IEEE_FE_GUIDE_MHZ, color=_IEEE_FE_COLOR, ls="--", lw=0.85, alpha=0.78, zorder=2)

    x_min, x_max = ax.get_xlim()
    y_min, _ = ax.get_ylim()
    pareto_x = cpu_arr[pareto]
    pareto_y = mae_arr[pareto]
    ...
    x_text = 10 ** (0.92 * np.log10(x_min) + 0.08 * np.log10(x_max))
    ax.text(
        x_text,
        _IEEE_FE_GUIDE_MHZ * 1.07,
        "IEEE FE guide",
        fontsize=7.9,
        color=_IEEE_FE_COLOR,
        va="bottom",
        ha="left",
        zorder=3,
    )
    ...
    family_handles.extend([
        ...
        Line2D([0], [0], color=_IEEE_FE_COLOR, ls="--", lw=0.85, label="IEEE FE guide"),
    ])

    ax.tick_params(axis="both", which="both", labelsize=9.5)
    ax.set_xlabel(r"CPU Time [$\mu$s/sample]", fontsize=10.5, labelpad=2)
    ax.set_ylabel("Mean MAE [mHz]", fontsize=10.5, labelpad=2)
    ax.set_title("(g) Accuracy vs. Latency", fontweight="bold", fontsize=11.5, loc="left", pad=5)
