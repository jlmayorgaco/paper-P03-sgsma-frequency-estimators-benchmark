...
\vspace{0.10em}
\callout{No single column predicts the others. EKF and IpDFT pass step/modulation yet \textbf{intermittently fail the ramp} (divergence); windowed/spectral methods fail the phase jump; and \emph{no} method survives the multi-event sequence. Standard tests alone leave IBR readiness untested.}
\end{frame}
...
\small
\begin{itemize}
  \item Core 5-scenario results: tuned comparison evidence ($n=5$).
  \item ATLAS severity sweeps: \textbf{paper-grade} --- $n=30$, fixed policy, both signs, \textbf{9/9} required sweeps (full-ATLAS readiness met).
  \item Journal-grade ($n\ge100$, archived hashes) is the remaining step.
\end{itemize}
\end{block}
\callout{The ATLAS severity results are now paper-grade; the journal-grade campaign ($n\ge100$) and the subspace/ML slow-estimator sweep are the remaining steps.}
\end{columns}
\end{frame}
...
\end{block}
\end{columns}
\callout{UKF shows the same intermittent divergence on the 5 and 15 Hz/s ramps; the robustified state-space variant stays stable across the whole ramp family.}
...
\begin{enumerate}
  \item<1-> \textbf{Latency and stress trade-off}: IpDFT/TFT provide strong steady-state accuracy but carry 3--5 cycle structural delay; loop methods are fast but can suffer elevated trip exposure.
  \item<2-> \textbf{Robustness beats point accuracy}: EKF, UKF and IpDFT intermittently diverge on ramps and multi-event stress (tens of Hz on individual seeds), while RA-EKF's explicit $\dot\omega$ state keeps it stable across every seed.
  \item<3-> \textbf{Compliance tests are insufficient}: estimators that pass standard-style Step/Ramp/Mod cases can fail under composite islanding and multi-event IBR stress.
\end{enumerate}
\vspace{0.4em}
\onslide<4->{\callout{Estimator qualification includes transient recovery, trip exposure, compute feasibility, and standard-case RMSE.}}
\end{frame}
...
\begin{columns}[T,onlytextwidth]
\column{0.49\linewidth}
\begin{block}{Full-microgrid fault validation \;\textcolor{ofbGreen}{\scriptsize[in progress]}}
\small
Integrate Simulink simulation outputs (CSV) from the Chamorro \emph{et al.} complete-microgrid fault model, replaying realistic fault waveforms through the same estimator pool.
\end{block}
\begin{block}{Modern and ML-based estimators}
\small
...
\end{block}
\column{0.49\linewidth}
\begin{block}{C++ acceleration}
\small
Port the heavy estimators and 1\,MHz signal generation to native C++ modules to cut benchmark runtime and enable journal-grade ($n\ge100$) Monte Carlo at scale.
\end{block}
\begin{block}{ANDES co-simulation}
\small
...
\end{columns}
\vspace{0.30em}
\callout{These extensions move OpenFreqBench from synthetic stress tests toward field-realistic, validated estimator qualification.}
\end{frame}
