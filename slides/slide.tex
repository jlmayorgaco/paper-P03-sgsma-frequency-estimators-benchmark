...
\node[result, fill=ofbSoft, draw=ofbBlue] at (0,0.28) {Ramp / RoCoF\\phase jump\\harmonics\\multi-event};
\node[result, fill=ofbSoft, draw=ofbGreen] at (3.15,0.28) {RMSE\\RoCoF error\\$T_{\mathrm{trip}}$\\settling};
\node[result, fill=ofbSoft, draw=ofbGold] at (6.30,0.28) {MCU/DSP\\FPGA\\CPU/GPU\\latency budget};
\node[result, fill=ofbSoft, draw=ofbViolet] at (9.45,0.28) {core table\\60-run sweep\\ATLAS pilot\\readiness gate};
...
\draw[arr] (event.south) to[out=-70,in=170] (decision.west);
\draw[arr] (metric.south) -- (decision.north);
\draw[arr] (cost.south) to[out=-110,in=10] (decision.east);
\draw[arr] (evidence.south) to[out=-105,in=5] (decision.east);
\node[result, fill=ofbBlue!7, draw=ofbBlue, minimum width=10.2cm] at (4.72,-2.28)
{Example: RA-EKF is recommended for ramp/phase-jump protection metrics under DSP/FPGA-class cost; it is not claimed as a universal RMSE winner.};
\end{tikzpicture}%
}
\end{frame}
...
\begin{block}{What the plot says}
\scriptsize
\begin{itemize}
  \item Scenario D exposes phase-reset behavior: RA-EKF reduces EKF exposure from 165 ms to 0.6 ms.
  \item Scenario E is multi-objective: RMSE and trip-risk do not select the same method.
  \item SRF-PLL is strong in Scenario D but reaches 471 ms in Scenario E.
\end{itemize}
\end{block}
{\scriptsize\color{ofbMuted}\textbf{Use:} $T_{\mathrm{trip}}$ is a first-class metric, not a footnote below RMSE.}
\end{columns}
\end{frame}
