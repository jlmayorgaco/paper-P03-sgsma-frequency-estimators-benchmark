...
\node[dot] at (7.8,0.05) {};
\node[tag] at (7.8,0.42) {2022};
\node[event, fill=ofbRed!9, draw=ofbRed] at (7.8,-1.15) {
\textbf{CA BESS}\\
Mar. 9 / Apr. 6\\[0.25em]
...
\node[tag] at (10.2,0.42) {2021};
\node[event, fill=ofbViolet!10, draw=ofbViolet] at (10.2,-1.15) {
\textbf{Kaua'i}\\
Nov. 21\\[0.25em]
...
\end{block}
\end{columns}
\artifact{Dong et al., Kaua'i 18--20 Hz oscillation analysis; NERC IBR monitoring white paper; IEC/IEEE 60255-118-1}
\end{frame}
...
\begin{frame}{Scenario suite I --- standard stresses}
\centering
\vspace{0.2em}
\includegraphics[width=\linewidth,height=0.80\textheight,keepaspectratio]{figures/scenario_suite_1.pdf}
\end{frame}

\begin{frame}{Scenario suite II --- composite IBR stresses}
\centering
\vspace{0.2em}
\includegraphics[width=\linewidth,height=0.80\textheight,keepaspectratio]{figures/scenario_suite_2.pdf}
\end{frame}
...
PLL & \cellcolor{ofbGreen!25}0.033 & \cellcolor{ofbRed!25}\textbf{1.47} & \cellcolor{ofbRed!25}\textbf{0.227} \\
IpDFT & \cellcolor{ofbGreen!25}0.000 & \cellcolor{ofbRed!35}\textbf{14.4} & \cellcolor{ofbRed!18}0.114 \\
EKF & \cellcolor{ofbGreen!25}0.000 & \cellcolor{ofbRed!35}\textbf{37.1} & \cellcolor{ofbGold!22}0.064 \\
\bottomrule
\end{tabular}
\vspace{0.15em}
...
\end{columns}
\vspace{0.1em}
\callout{\textbf{Result:} RA-EKF, SOGI-FLL, Koopman, and TFT remain below 0.05 Hz across the three standard cases; EKF and IpDFT fail through intermittent ramp divergence.}
\end{frame}

\begin{frame}{Scenario D: phase jump separates filters from windowed methods}
\begin{columns}[T,onlytextwidth]
\column{0.49\linewidth}
\begin{block}{State-space + loops recover cleanly}
\scriptsize
A clean $+60^\circ$ phase jump (NERC profile) is absorbed with near-zero error and \textbf{zero trip exposure}:
\begin{itemize}\setlength\itemsep{2pt}
  \item EKF, UKF, RA-EKF: RMSE $\approx 0$ Hz, $T_{\mathrm{trip}}=0$.
  \item PLL 0.021 Hz, SOGI-FLL 0.067 Hz, $T_{\mathrm{trip}}=0$.
\end{itemize}
\end{block}
\column{0.49\linewidth}
\begin{block}{Windowed / spectral methods trip}
\scriptsize
The trip exposure here lives in the window/spectral family:
\begin{itemize}\setlength\itemsep{2pt}
  \item IpDFT: 1.56 Hz RMSE, $T_{\mathrm{trip}}=1.21$ s.
  \item RLS 2.28 Hz / 1.30 s; Prony 2.39 Hz / 1.35 s.
  \item TFT 0.43 Hz / 0.09 s; Koopman 0.39 Hz / 0.11 s.
\end{itemize}
\end{block}
\end{columns}
\vspace{-0.10cm}
{\scriptsize\color{ofbMuted}\textbf{Benchmark reading:} an isolated phase jump does \emph{not} break the state-space filters --- EKF already recovers with zero trip exposure. RA-EKF's real advantage appears on ramps and multi-event stress (next), not on clean phase jumps.}
\artifact{full\_mc\_benchmark, $n=5$: NERC\_Phase\_Jump\_60}
\end{frame}
...
\centering
\vspace{-0.25em}
\includegraphics[width=0.96\paperwidth,height=0.80\textheight,keepaspectratio]{figures/event_responses_mc.pdf}
\end{frame}
...
\midrule
Koopman & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F & \cellcolor{ofbRed!22}F \\
TFT & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGold!30}F* & \cellcolor{ofbRed!22}F \\
IpDFT & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F & \cellcolor{ofbRed!22}F \\
SOGI-FLL & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F \\
PLL & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F & \cellcolor{ofbGold!30}F* & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F \\
EKF & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F \\
UKF & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F \\
RA-EKF & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F \\
LKF2 & \cellcolor{ofbGold!30}F* & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbGreen!25}P & \cellcolor{ofbRed!22}F \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.10em}
...
{\scriptsize\color{ofbMuted}\textbf{Result:} Step, ramp, and modulation compliance does not predict phase-jump or multi-event readiness.}
\end{frame}
...
\end{columns}
{\scriptsize\color{ofbMuted}\textbf{Result:} The deliverable is a selection map: event class $\times$ metric $\times$ compute budget.}
\end{frame}
...
\end{columns}
{\small\textbf{Result:} aggregate rankings hide stress-specific collapse; the benchmark reports per-stress behavior, not one score.}
\end{frame}
...
\begin{enumerate}
  \item<1-> \textbf{Estimator choice is metric-conditioned}: across ATLAS severity levels, the RMSE leader often differs from the trip-risk and RoCoF leader.
  \item<2-> \textbf{Stress boundaries matter}: AM modulation remains survivable for many methods, while severe FM, RoCoF, interharmonics, noise, and extreme THD leave no method below the 0.05 Hz guide.
  \item<3-> \textbf{Robustness beats point accuracy}: EKF, UKF and IpDFT intermittently diverge on ramps and multi-event stress, while RA-EKF's explicit $\dot\omega$ state keeps the ramp family stable across seeds.
\end{enumerate}
\vspace{0.4em}
\onslide<4->{\callout{Estimator qualification includes event family, sign, severity, transient recovery, trip exposure, compute feasibility, and standard-case RMSE.}}
\end{frame}
