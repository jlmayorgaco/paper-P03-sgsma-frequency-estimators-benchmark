...
\node[contrib, fill=ofbViolet!8, draw=ofbViolet] (c4) at (7.15,-1.81) {\textbf{4. RA-EKF variant tested}\\explicit $\dot\omega$ state and event-gated adaptation};
...
\begin{frame}{Scenario families: distortion, leakage, and low SNR}
\vspace{-0.18em}
\begin{columns}[T,onlytextwidth]
...
\scriptsize
$n=30$, fixed policy, both signs, \textbf{9/9} required sweeps; \textbf{18-estimator} canonical set incl.\ MUSIC (PI-GRU experimental, GPU-bound, separate sweep). Readiness gate: \texttt{status=paper\_grade}, \texttt{scope=full\_atlas}.
\end{block}
...
\end{columns}
\vspace{0.2em}
\callout{The validated $n=30$ ATLAS run is complete (18 estimators); the $n\ge100$ replication run and the GPU-bound PI-GRU sweep are the remaining steps.}
...
How reliable are the ATLAS sweeps? & Validated $n=30$: fixed policy, both signs, 9/9 required sweeps, 18 estimators. The $n\ge100$ replication and the GPU-bound PI-GRU sweep remain. \\
