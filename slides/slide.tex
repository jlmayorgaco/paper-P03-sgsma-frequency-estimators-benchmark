...
\newcommand{\papergradebanner}{%
  ...
  \noindent\begin{tikzpicture}
    ...
    {PAPER-GRADE SWEEP: $n=30$, fixed policy, both signs --- full-ATLAS readiness still in progress};
  \end{tikzpicture}\par\vspace{-0.10cm}%
}
...
\end{columns}
\vspace{0.4em}
\callout{\textbf{Dataset}: 9 principal estimators, 2 legacy baselines, 5 stress scenarios, standard and IBR-specific metrics, plus an expanded sweep of 16 estimators, 34 scenarios, and 60 Monte Carlo runs per scenario.}
\end{frame}
...
\node<4->[result, fill=ofbSoft, draw=ofbViolet] (evidencer) at (9.45,0.28) {core table\\expanded sweep (60 runs)\\ATLAS ($n{=}1$)\\validation gate};
...
\centering
\vspace{-0.30em}
\includegraphics[width=0.99\linewidth,height=0.625\textheight,keepaspectratio]{figures/scenario_suite_1.pdf}
\vspace{-0.10em}
\callout{\textbf{Reading:} each column pairs the voltage waveform $v(t)$ (top) with the true frequency $f(t)$ (bottom) for the same case. Magnitude step and AM leave $f(t)$ flat at 60 Hz --- the stress is not a frequency event, yet estimators still err.}
\end{frame}

\begin{frame}{Scenario suite II --- composite IBR stresses}
\centering
\vspace{-0.30em}
\includegraphics[width=0.99\linewidth,height=0.625\textheight,keepaspectratio]{figures/scenario_suite_2.pdf}
\vspace{-0.10em}
\callout{\textbf{Reading:} harmonics, phase jumps and out-of-band interference also keep $f(t)$ near 60 Hz; only the ringdown and multi-event sequence move the true frequency. The waveform $v(t)$ is where most IBR stress actually lives.}
\end{frame}
...
\end{center}
\vspace{0.10em}
\begin{center}{\scriptsize\color{ofbMuted}Columns: \textbf{Isl} = composite islanding (D), \textbf{Multi} = multi-event (E). \quad \textbf{P} pass \,\textbar\, \textbf{F} fails a declared RMSE/$T_{\mathrm{trip}}$/settling threshold \,\textbar\, \textbf{F*} borderline: meets trip-risk but misses the accuracy/settling target.}\end{center}
\vspace{0.10em}
...
\small
\begin{itemize}
  \item 16-estimator, 34-scenario, 10-family expanded sweep.
  \item 60 Monte Carlo runs per scenario.
...
\begin{enumerate}
  ...
  \item<2-> \textbf{State-space estimation matters}: RA-EKF's explicit $\dot\omega$ state plus event gating reduces Scenario-D trip exposure from 0.165 s for EKF to about 0.6 ms.
...
\node<2->[task, fill=ofbSoft, draw=ofbGreen] (m2) at (3.3,-0.35) {SRF-PLL / RA-EKF\\near-zero $T_{\mathrm{trip}}$};
\node<3->[task, fill=ofbSoft, draw=ofbGold] (m3) at (6.6,-0.35) {SRF-PLL / EKF\\CPU efficient};
