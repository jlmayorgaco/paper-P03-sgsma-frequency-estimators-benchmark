...
CA, Oct. 9\\[0.25em]
\textbf{682 / 937 MW}\\
solar reductions; NERC states frequency tripping was not the cause\\[0.25em]
...
\artifact{NERC Blue Cut 2017; NERC/WECC Canyon 2 2018; NERC Odessa 2021/2022; NERC/WECC CA BESS 2023; Dong et al. Kaua'i analysis}
\end{frame}
...
\statcard{ofbBlue}{18}{estimators}
\vspace{0.35em}
...
\end{columns}
\vspace{0.4em}
\callout{\textbf{Dataset}: 18 estimators (incl.\ legacy baselines) across 33 scenarios, $n=5$ Monte Carlo per scenario, with standard and IBR-specific metrics, plus ATLAS severity sweeps.}
\end{frame}
...
\node<4->[result, fill=ofbSoft, draw=ofbViolet] (evidencer) at (9.45,0.28) {full sweep ($n{=}5$)\\ATLAS severity\\($n{=}30$ paper-grade)\\validation gate};
...
\node[box, fill=ofbGold!18, draw=ofbGold] (est) at (5.6,1.2) {18 estimators\\common scenarios};
...
\midrule
ZCD & 7 $\mu$s & Loop & MCU \\
SOGI-FLL & 9 $\mu$s & Loop & MCU \\
PLL & 9 $\mu$s & Loop & MCU \\
EKF & 11 $\mu$s & Model & MCU/DSP \\
UKF & 13 $\mu$s & Model & DSP \\
RA-EKF & \textbf{15 $\mu$s} & \textbf{Model} & \textbf{DSP} \\
IpDFT & 17 $\mu$s & Window & DSP/FPGA \\
ESPRIT & $\approx$1{,}400 $\mu$s & Window & CPU \\
Koopman & $\approx$15{,}000 $\mu$s & Data-driven & CPU/GPU \\
\bottomrule
\end{tabular}
...
\small
The state-space and loop families all run at $\sim$10--17 $\mu$s/sample (embedded-feasible); spectral (ESPRIT, MUSIC) and data-driven (Koopman) methods are 100--1000$\times$ costlier and data-dependent.
\end{block}
\end{columns}
\artifact{full\_mc\_benchmark, $n=5$: mean \texttt{m13\_cpu\_time\_us}}
\end{frame}
...
\item State-space + loop methods cluster at low cost ($\le17\,\mu$s) and low trip.
\item Spectral/data-driven methods trade lower error for 100--1000$\times$ higher CPU.
...
Under multi-event stress UKF has the lowest RMSE/trip, SOGI-FLL the lowest usable CPU, and Koopman is $1000\times$ costlier.
\end{block}
\begin{block}{Deployability}
\scriptsize
\textbf{Feasibility changes the accuracy ranking.}\\
RA-EKF runs at $\sim$15 $\mu$s/sample; the spectral and data-driven methods are 100--1000$\times$ costlier and sit outside relay-class timing.
\end{block}
\end{columns}
\vspace{-0.10cm}
...
\begin{itemize}
  ...
  \item Fixed-policy, paper-grade $n=30$ across all 9 required sweeps (both signs).
...
\end{tabular}
\vspace{0.25em}
\callout{The full Monte-Carlo sweep confirms the central pattern: the RMSE leader rotates (RA-EKF, UKF, LKF2, SOGI-FLL) by disturbance family, and error grows from sub-mHz on clean cases to $\sim$1 Hz under multi-event stress.}
\end{frame}
...
\midrule
RMSE vs trip-risk & \cellcolor{ofbRed!18}83 & 113 \\
RMSE vs RoCoF max error & \cellcolor{ofbGold!18}55 & 113 \\
RMSE vs RoCoF RMS error & \cellcolor{ofbGold!18}50 & 113 \\
\bottomrule
\end{tabular}
\vspace{0.35em}
\begin{block}{Winner concentration}
\scriptsize
...
\end{block}
\end{columns}
{\small\textbf{Result:} across 113 severity levels, the preferred estimator changes when the objective changes.}
...
\insightcard{ofbBlue}{1. Metric-conditioned}{The RMSE leader often differs from the trip-risk and RoCoF leader across ATLAS severity levels.}
\column{0.32\linewidth}
...
\column{0.32\linewidth}
\insightcard{ofbGreen}{3. Robustness over point accuracy}{EKF, UKF, and IpDFT can diverge on ramps; RA-EKF keeps the ramp family stable across seeds.}
\end{columns}
\vspace{0.5em}
...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \node[anchor=east, text=white, align=right, font=\large, text width=0.62\paperwidth] at ([xshift=-0.075\paperwidth,yshift=-1.00cm]current page.east) {Metrics, extra stress maps, the journal-grade plan, references.};
\end{tikzpicture}
\end{frame}
...
\begin{frame}{Backup: from paper-grade (done) to journal-grade}
\begin{columns}[T,onlytextwidth]
\column{0.50\linewidth}
\begin{block}{Completed --- paper-grade}
\scriptsize
...
\end{block}
\begin{block}{Next --- journal-grade}
\scriptsize
...
\scriptsize
\begin{itemize}
  \item Paper-grade $n=30$ campaign: \textbf{done} (readiness gate passed).
  \item Journal-grade lifts to $n\ge100$ with archived seeds, manifests, and hashes.
...
\end{columns}
\vspace{0.2em}
\callout{Paper-grade ($n=30$, full ATLAS) is complete; the $n\ge100$ journal-grade run and the subspace/ML sweep are the remaining steps.}
...
\node[claim, fill=ofbBlue!12, draw=ofbBlue] (b) at (2.35,0.85) {Benchmark\\18 estimators};
...
\end{columns}
\vspace{0.45em}
\callout{The core benchmark supports the main empirical conclusions; the paper-grade ATLAS sweeps ($n=30$) explain mechanisms, with the journal-grade run as the next step.}
\end{frame}
...
\vspace{0.35em}
\begin{itemize}
  \item ZCD is the strongest method under isolated integer-harmonic stress at low THD, but the paper-grade $n=30$ sweep shows it collapses to tens of Hz above $\sim$20\% THD.
...
How reliable are the ATLAS sweeps? & Paper-grade: $n=30$, fixed policy, both signs, 9/9 required sweeps (full-ATLAS readiness met). Journal-grade ($n\ge100$) and the subspace/ML sweep remain. \\
