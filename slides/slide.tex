...
\begin{frame}{IBR disturbances expose a measurement gap}
\tiny
\renewcommand{\arraystretch}{1.05}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.39\linewidth}>{\raggedright\arraybackslash}p{0.30\linewidth}}
\toprule
\textbf{Event} & \textbf{Verified behavior} & \textbf{Benchmark implication} \\
\midrule
Blue Cut Fire, CA, Aug. 16, 2016 & Fault-induced solar PV interruption near 1.2 GW; NERC tied erroneous trips to near-instantaneous frequency estimates during distorted fault waveforms. & Test whether waveform distortion is misread as frequency collapse. \\
Canyon 2 Fire, CA, Oct. 9, 2017 & Two normally cleared line-to-line faults caused 682 MW and 937 MW solar PV reductions. NERC states frequency-related tripping was \emph{not} the cause. & Separate frequency tracking from voltage, phase-lock, momentary-cessation, and recovery artifacts. \\
Odessa, TX, May 9/Jun. 26, 2021; Jun. 4, 2022 & Normally cleared Texas faults caused widespread solar PV reductions: 1,112 MW in 2021 and 1,711 MW in 2022, including plants far from the initiating fault. & Include phase jumps, fast controls, model-mismatch regimes, and event-specific trip-risk. \\
California BESS, Mar. 9/Apr. 6, 2022 & Normally cleared Western Interconnection faults produced abnormal BESS/IBR active-power reductions; NERC lists ac overcurrent/overvoltage, dc-bus, unbalanced-current, and data-quality issues. & Evaluate estimators under low-SNR, unbalanced, and non-sinusoidal conditions. \\
\bottomrule
\end{tabular}
\vspace{0.15em}
\artifact{NERC Blue Cut 2017; NERC/WECC Canyon 2 2018; NERC Odessa 2021/2022; NERC/WECC CA BESS 2023}
\end{frame}
...
\begin{center}
\begin{tabular}{>{\raggedright\arraybackslash}p{0.22\linewidth}rrr}
\toprule
Method & Step $+10\%$ & Ramp & Modulation 2 Hz \\
 & RMSE [Hz] & RMSE [Hz] & RMSE [Hz] \\
\midrule
IpDFT & \textbf{0.000} & \textbf{14.4} & 0.114 \\
TFT & 0.001 & 0.019 & 0.046 \\
Koopman & 0.003 & 0.017 & 0.044 \\
\midrule
PLL & 0.033 & \textbf{1.47} & \textbf{0.227} \\
SOGI-FLL & 0.005 & 0.013 & 0.040 \\
\midrule
EKF & \textbf{0.000} & \textbf{37.1} & 0.064 \\
UKF & \textbf{0.000} & 0.017 & 0.055 \\
RA-EKF & 0.000 & \textbf{0.006} & 0.037 \\
LKF2 & 0.138 & 0.015 & \textbf{0.028} \\
\bottomrule
\end{tabular}
\end{center}
\vspace{-0.05cm}
\callout{Reading (mean over 5 seeds): RA-EKF, SOGI-FLL, Koopman and TFT stay $\le0.05$ Hz on all three, but \textbf{EKF and IpDFT intermittently diverge on the ramp} (37 and 14 Hz) and PLL degrades on ramp/modulation. Robustness, not point accuracy, separates the families.}
\vspace{-0.05cm}
\artifact{full\_mc\_benchmark, $n=5$}
\end{frame}
...
\item Spectral/data-driven methods (MUSIC, ESPRIT, Koopman) are 100--1000$\times$ costlier; RLS/Prony pair high trip with low accuracy.
\item RA-EKF sits in the low-cost, never-diverging region.
\end{itemize}
\end{exampleblock}
\end{columns}
\end{frame}
...
\toprule
Stress family & Maximum tested level & Methods below 0.05 Hz & Reading \\
\midrule
