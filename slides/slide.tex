...
\oralcompacttrue
\iforalcompact
\includeonlyframes{title,fieldevents,researchproblem,operatingclaim,technicalcontributions,experimentscale,openfreqbenchplatform,experimentalmethod,fullworkflow,scenariofamilies,estimatorfamilies,metrics,standardresults,compositeibr,resultcomparison,computationalfeasibility,complianceheatmap,atlasseverity,amfminterharmonics,textbookdivergence,wholestressspace,computecost,threefindings,recommendationmap,validationconclusion}
\fi
...
\begin{frame}{Standard stresses (1 of 2): magnitude, ramp, frequency step}
\vspace{-0.15em}
...
\begin{frame}{Standard stresses (2 of 2): modulation (AM and FM)}
\vspace{-0.15em}
\evidencetag{ofbBlue}{CORE SCENARIO SUITE}
\begin{center}
...
\begin{frame}{Composite IBR stresses (1 of 2): harmonics, phase jump, out-of-band}
\vspace{-0.15em}
...
\begin{frame}{Composite IBR stresses (2 of 2): ringdown and multi-event}
\vspace{-0.15em}
\evidencetag{ofbRust}{IBR STRESS SUITE}
\begin{center}
...
\begin{frame}{How estimators track an IBR ringdown}
\evidencetag{ofbViolet}{FULL MONTE-CARLO}
\begin{center}
\includegraphics[width=0.97\linewidth,height=0.74\textheight,keepaspectratio]{figures/ringdown_tracking_rep.pdf}
\end{center}
\vspace{-0.3em}
\resultclaim{ofbBlue}{Black is the true frequency, red is each estimator on the power-imbalance ringdown. Robust methods (RA-EKF, SOGI-FLL) follow the damped 59--61 Hz swing; fragile ones (ZCD) overshoot or run away.}
\end{frame}
...
\begin{frame}{Safe-operating envelope: where each estimator stays usable}
\papergradebanner
\begin{center}
\includegraphics[width=0.99\linewidth,height=0.80\textheight,keepaspectratio]{figures/atlas_safe_envelope.pdf}
\end{center}
\vspace{-0.35em}
\resultclaim{ofbGreen}{Each cell is the severity at which frequency RMSE first exceeds 0.05 Hz, in that family's own units ($+$ = never exceeded). This is a deployable map of operating limits, not a single ranking.}
\end{frame}
...
\scriptsize
$n=30$, fixed policy, both signs, \textbf{9/9} required sweeps; \textbf{18-estimator} canonical set incl.\ MUSIC (PI-GRU experimental, GPU-bound, separate sweep). Current audit: \texttt{status=diagnostic}; Prony has non-finite outputs in selected stress-corner cells.
\end{block}
...
\scriptsize
\begin{itemize}
  \item $n=30$ ATLAS campaign: \textbf{done}; readiness gate flags Prony stress-corner finite-coverage failures.
  \item The $n\ge100$ replication layer adds archived seeds, manifests, and hashes.
  \item MUSIC and ESPRIT are now in the canonical set; only PI-GRU (GPU-bound) remains a separate experimental sweep.
  \item Every table and figure regenerates from artifacts.
\end{itemize}
\end{columns}
\vspace{0.2em}
\callout{The $n=30$ ATLAS run is complete as an extended stress map; final qualification needs Prony stress-cell remediation or explicit exclusion criteria, plus the $n\ge100$ replication layer.}
...
How reliable are the ATLAS sweeps? & Extended $n=30$: fixed policy, both signs, 9/9 required sweeps, 18 estimators. Current audit is diagnostic because Prony has non-finite stress-corner outputs; the $n\ge100$ replication and GPU-bound PI-GRU sweep remain. \\
