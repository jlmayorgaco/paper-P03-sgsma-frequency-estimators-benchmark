...
This review covers the MVP 2.0.0 package at the repository root. The `src/`
tree is now the public package source, including the audited paper benchmark
pipelines that remain necessary for reproducing the study.
...
- The public branch now contains only the package line; historical layouts remain in older branches.
...
to split as the platform grows.
...
The paper and sweep pipelines under `pipelines/` are large scripts. Do not
