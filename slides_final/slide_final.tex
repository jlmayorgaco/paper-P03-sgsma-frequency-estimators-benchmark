...
\input{../slides/generated_results.tex}
...
\begin{tikzpicture}[
  ...
  contrib/.style={draw, rounded corners=3pt, very thick, align=left, text width=8.6cm, minimum height=1.0cm, inner xsep=10pt, inner ysep=6pt}
]
% Four contributions, centered, distinct accents, comfortable vertical spacing
\node[contrib, fill=ofbBlue!7, draw=ofbBlue]    (c1) at (0,1.30)  {\textbf{1. OpenFreqBench}\\reproducible stress-test platform};
\node[contrib, fill=ofbGreen!7, draw=ofbGreen]   (c2) at (0,0.10)  {\textbf{2. Metric-complete protocol}\\RMSE, peak/RoCoF error, $T_{\mathrm{trip}}$, settling, CPU};
\node[contrib, fill=ofbRust!8, draw=ofbRust]     (c3) at (0,-1.10) {\textbf{3. IBR failure evidence}\\standard cases miss composite IBR failures};
\node[contrib, fill=ofbViolet!8, draw=ofbViolet] (c4) at (0,-2.30) {\textbf{4. Extensible estimator contract}\\new methods enter the same scoring workflow};
\node[draw=ofbBlue, fill=ofbBlue!6, rounded corners=3pt, line width=0.8pt, align=left, text width=8.9cm, inner xsep=10pt, inner ysep=6pt] at (0,-3.55) {%
...
\node[hero, fill=ofbViolet!9, draw=ofbViolet] (m) at (9.45,1.30) {{\fontsize{25}{27}\selectfont\bfseries\color{ofbViolet}30}\\[0.20em]\textbf{seeds / pair}\\[0.12em]{\color{ofbMuted}Monte-Carlo}};

\node[strip, fill=ofbSoft, draw=ofbLine, text width=11.3cm] (d) at (4.725,-0.78) {%
\textbf{18\,\(\times\)\,33 \(=\) 594} estimator--scenario cells \;\textcolor{ofbMuted}{\textbullet}\; \textbf{594\,\(\times\)\,30 \(=\) 17{,}820} Monte-Carlo records \;\textcolor{ofbMuted}{\textbullet}\; \textbf{1\,MHz} truth \(\to\) \textbf{10\,kHz} output%
};
...
PLL & \cellcolor{ofbGreen!25}0.033 & \cellcolor{ofbGreen!25}0.073 & \cellcolor{ofbRed!25}\textbf{0.227} \\
IpDFT & \cellcolor{ofbGreen!25}0.000 & \cellcolor{ofbGreen!25}0.037 & \cellcolor{ofbRed!18}0.114 \\
EKF & \cellcolor{ofbGreen!25}0.000 & \cellcolor{ofbGreen!25}0.010 & \cellcolor{ofbGold!22}0.064 \\
\bottomrule
\end{tabular}
\vspace{0.15em}
{\tiny\color{ofbMuted}RMSE [Hz], mean over 30 seeds. Green: below the 0.05 Hz guide; red: above it.\\[0.25em]LKF2 implementation after H.~Ahmed, S.~Biricik, M.~Benbouzid, ``Linear Kalman Filter-Based Grid Synchronization Technique: An Alternative Implementation,'' IEEE, 2021.}
...
\small
Under the stacked sequence every method converges to $\approx1.1$ Hz RMSE. High accuracy is simply not available in this regime.
\end{block}
\column{0.47\linewidth}
...
\end{block}
\end{columns}
\artifact{Monte-Carlo benchmark, $n=30$ seeds --- composite multi-event scenario}
\end{frame}
...
Ramp and RoCoF tracking & RA-EKF leads; the clean ramp is tracked by all & RA-EKF 0.004 Hz; EKF/UKF/IpDFT all $<0.05$ Hz & RoCoF \emph{magnitude} is the real stressor (severity curves) \\
...
Multi-event RMSE & No accurate method --- all $\approx1.1$ Hz & UKF 1.09 Hz best; wide spread across methods & RMSE hides trip-risk and $1000\times$ CPU spread \\
...
\bottomrule
\end{tabular}\\[0.35em]
{\tiny\color{ofbMuted}$\dagger$ mean per-sample wall-clock in the Python harness (software proxy), $n=30$; Koopman/ESPRIT scale with event length.}
...
\papergradebanner
\centering
{\scriptsize\color{ofbMuted}Error grows with RoCoF magnitude and phase-jump angle; the curve slope and onset are the family-specific response signatures.}
\vspace{0.25em}
