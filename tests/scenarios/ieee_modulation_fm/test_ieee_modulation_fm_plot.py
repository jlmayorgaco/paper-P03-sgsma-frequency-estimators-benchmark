...
def test_ieee_modulation_fm_plot_and_csv():
    out_dir = ROOT / "tests" / "scenarios" / "ieee_modulation_fm" / "artifacts"
    out_dir.mkdir(parents=True, exist_ok=True)

    sc = IEEEModulationFMScenario.run(
        duration_s=2.0,
        freq_nom_hz=F_NOM,
        ka=0.1,         # 0.1 rad de modulación PM/FM
    ...
    csv_path = out_dir / "ieee_modulation_fm.csv"
    ...
    # Gráfico de Voltaje (Bloque SÓLIDO constante)
    axes[0].plot(sc.t, sc.v, linewidth=0.5, color='darkblue')
    axes[0].set_ylabel("Voltage [pu]")
    axes[0].set_title(f"{IEEEModulationFMScenario.get_name()} (Pure PM/FM at 2.0 Hz)")
    axes[0].set_ylim(-1.3, 1.3)

    # Gráfico de Frecuencia (Se verá la oscilación)
    ...
    png_path = out_dir / "ieee_modulation_fm.png"
