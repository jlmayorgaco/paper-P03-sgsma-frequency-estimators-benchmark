...
def test_ieee_modulation_plot_and_csv():
    out_dir = ROOT / "tests" / "scenarios" / "ieee_modulation" / "artifacts"
    ...
    sc = IEEEModulationScenario.run(
        ...
        ka=0.1,         # 0.1 rad de modulación PM
        fm_hz=2.0,      # Modulación a 2 Hz (oscilación clásica inter-área)
        noise_sigma=0.0,# Limpio para la gráfica
        seed=42,
    )

    df = pd.DataFrame({"t_s": sc.t, "v_pu": sc.v, "f_true_hz": sc.f_true})
    csv_path = out_dir / "ieee_modulation.csv"
    ...
    axes[0].set_title(f"{IEEEModulationScenario.get_name()} (AM + PM at 2.0 Hz)")
    axes[0].set_ylim(-1.3, 1.3)

    # Gráfico de Frecuencia (se verá la onda FM)
    axes[1].plot(sc.t, sc.f_true, linewidth=1.5, color='red', label="True Frequency")
    axes[1].set_xlabel("Time [s]")
    axes[1].set_ylabel("Frequency [Hz]")
    axes[1].legend(loc="upper right")

    png_path = out_dir / "ieee_modulation.png"
