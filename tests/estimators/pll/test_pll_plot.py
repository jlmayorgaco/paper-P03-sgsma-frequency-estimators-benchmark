...
def test_pll_vs_zcd_visual_diagnostic():
    out_dir = ROOT / "tests" / "estimators" / "pll" / "artifacts"
    ...
    noise = rng.normal(0.0, 0.00000000000000005, size=len(t))
    ...
    axes[1].plot(
        ...
        linewidth=2.0,
        color="crimson",
        label="PLL (simple multiplier-based)",
    )
    axes[1].axvline(
        t_step,
        color="gray",
    ...
    axes[1].set_title("Dynamic Tracking: ZCD Latency vs. Simple PLL")
    axes[1].set_ylim(48.0, 54.5)
    axes[1].grid(True, alpha=0.3)
    axes[1].legend(loc="lower right")

    plot_path = out_dir / "pll_vs_zcd_diagnostic.png"
