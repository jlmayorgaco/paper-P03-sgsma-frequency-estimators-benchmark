...
def test_lkf_vs_zcd_visual_diagnostic():
    out_dir = ROOT / "tests" / "estimators" / "lkf" / "artifacts"
    ...
    t_step = 0.1
    f_true = np.where(t < t_step, 50.0, 55.0)
    ...
    lkf = LKF_Estimator(
        ...
        r=1.5e-2,
        rho=0.9995,
        output_smoothing=0.04,
    )
    f_lkf = lkf.estimate(t, v)
    ...
    axes[1].plot(
        ...
        linewidth=2.0,
        color="crimson",
        label="LKF (phase-increment frequency estimate)",
    )
    axes[1].axvline(
        t_step,
        color="gray",
    ...
    axes[1].set_title("Dynamic Tracking: ZCD Latency vs. LKF Smooth Tracking")
    axes[1].set_ylim(48.0, 65.0)
    axes[1].grid(True, alpha=0.3)
    axes[1].legend(loc="lower right")

    plot_path = out_dir / "lkf_vs_zcd_diagnostic.png"
