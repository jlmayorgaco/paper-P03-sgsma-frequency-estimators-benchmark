...
def test_ekf_nominal_pure_sine():
    """
    ...
    assert np.isclose(f_mean, 50.0, atol=0.1)
    assert np.percentile(abs_err, 95) < 0.25


def test_ekf_step_tracking():
    """
    ...
    assert np.isclose(np.mean(f_post), 52.0, atol=0.25)
    assert np.percentile(abs_err, 95) < 0.6


def test_ekf_robustness_to_noise():
    """
