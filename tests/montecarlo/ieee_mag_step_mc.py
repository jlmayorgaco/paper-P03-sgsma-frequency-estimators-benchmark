...
# Importamos el nuevo escenario de salto de magnitud
...
def test_a_b_classes_and_engine_loading():
    ...
    assert IEEEMagStepScenario is not None, "La clase del escenario no se cargó."
    ...
    assert engine.scenario_cls == IEEEMagStepScenario
...
def test_c_single_run_success():
    ...
    assert result.scenario_name == IEEEMagStepScenario.get_name()
...
def test_d_ten_runs_artifacts_and_metrics():
    ...
    # Cambiamos la ruta de salida para no sobreescribir el escenario anterior
    out_dir = ROOT / "tests" / "montecarlo" / "artifacts" / "ieee_mag_step_ipdft"
